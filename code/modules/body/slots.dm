...
//
// Until equip is a ledger move, legacy equips land in the default `body` slot
// and the per-slot vars say what is worn. body_slot_item() reads the ledger
// slot first and falls back to the legacy accessor, so everything reading
// through it is right on both paths.
...
/datum/slot_def/body
	...
	capacity = 1
	// The mob's own Destroy() decides what happens to its equipment until the
	// u_equip chain goes.
	...
	/// slot_* number the legacy inventory uses for this slot, or null.
	var/legacy_slot
	/// BODY_SLOT_* roles.
	var/roles = NONE
...
/// The item in this slot according to the legacy inventory, or null.
/datum/slot_def/body/proc/legacy_item(mob/living/holder)
	return legacy_slot ? holder.get_equipped_item(legacy_slot) : null
...
/datum/slot_def/body/module
	/// Which module_state_N var the legacy robot code keeps it in.
	var/module_index

/datum/slot_def/body/module/legacy_item(mob/living/holder)
	var/mob/living/silicon/robot/R = holder
	if(!istype(R))
		return null
	switch(module_index)
		if(1)
			return R.module_state_1
		if(2)
			return R.module_state_2
		if(3)
			return R.module_state_3
	return null
...
/// What is in body slot `def`: the ledger slot, else the legacy inventory.
/mob/living/proc/body_slot_item(datum/slot_def/body/def)
	var/list/things = ledger?.slots[def.id]
	if(length(things))
		return things[1]
	return def.legacy_item(src)
