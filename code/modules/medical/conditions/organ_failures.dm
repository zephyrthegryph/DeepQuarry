...
// base /datum/affliction: the metric or organ-damage
...
/datum/affliction/hepatic_failure
	...
	treated_by = list(TREAT_HEPATORENAL = 1.2)
	symptom_pool = list(
	...
	min_symptoms = 2
	max_symptoms = 3
	// Toxin clearance failure. We stack toxin injury while present; the
...
/datum/affliction/renal_failure
	...
	treated_by = list(TREAT_HEPATORENAL = 1.2, TREAT_BLOOD_RESTORE = 0.2)
	symptom_pool = list(
		/datum/affliction_symptom/fatigue        = 90,
	...
	// model that as slow toxin injury accumulation.
	organ_damage_threshold = 0
	organ_damage_type = INJURY_TOXIN
...
/datum/affliction/ischemic_vision_loss
	...
	treated_by = list(TREAT_OCULAR = 1.0)
	symptom_pool = list(
...
/datum/affliction/heart_damage
	...
	treated_by = list(TREAT_CARDIAC = 1.0, TREAT_CIRCULATORY = 0.4)
	worsened_by_tags = list(TREAT_STIMULANT = 1.0)
