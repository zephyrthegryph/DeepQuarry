...
/proc/attempt_to_devour_prey(mob/living/user, mob/living/prey, mob/living/pred, obj/belly/belly, delay_time)
	...
	if(!user.client && prey.get_weakened() > 0) // stop crwaling instantly break swallow attempt for mobvore
		prey.Stun(min(prey.get_weakened(), 2)) // stop crawling instantly break swallow attempt for mobvore
...
/proc/devour_and_move_prey(mob/living/user, mob/living/prey, mob/living/pred, obj/belly/belly, message_range)
	...
	if(istype(victim_pred) && !victim_pred.client && !victim_pred.ai_brain && victim_pred.ckey)
