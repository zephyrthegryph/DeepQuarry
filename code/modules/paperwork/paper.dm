...
/obj/item/paper/attackby(obj/item/P, mob/user)
	...
	if(istype(P, /obj/item/paper) || istype(P, /obj/item/photo))
		...
		if(ishuman(user))
			...
			if (h_user.get_equipped_item(SLOT_ID_R_HAND) == src)
				h_user.drop_from_inventory(src)
				h_user.put_in_r_hand(B)
			else if (h_user.get_equipped_item(SLOT_ID_L_HAND) == src)
				h_user.drop_from_inventory(src)
				h_user.put_in_l_hand(B)
			else if (h_user.get_equipped_item(SLOT_ID_L_STORE) == src)
				h_user.drop_from_inventory(src)
				if(!h_user.equip_to_slot_if_possible(B, slot_l_store))
					h_user.drop_from_inventory(B)
			else if (h_user.get_equipped_item(SLOT_ID_R_STORE) == src)
				h_user.drop_from_inventory(src)
