...
/obj/item/paperplane/throw_impact(atom/hit_atom)
	...
	if(prob(2))
		if((H.get_equipped_item(SLOT_ID_HEAD) && H.get_equipped_item(SLOT_ID_HEAD).body_parts_covered & EYES) || (H.get_equipped_item(SLOT_ID_WEAR_MASK) && H.get_equipped_item(SLOT_ID_WEAR_MASK).body_parts_covered & EYES) || (H.get_equipped_item(SLOT_ID_GLASSES) && H.get_equipped_item(SLOT_ID_GLASSES).body_parts_covered & EYES))
			return
