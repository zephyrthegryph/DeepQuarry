...
/obj/item/dq_property_test
	...
	sharp = TRUE
	DEFAULT_MATTER(list(MAT_STEEL = 1000, MAT_CARDBOARD = 500))
...
/datum/unit_test/dq_property_item_families/Run()
	...
	// Matter-derived values per type, from the DEFAULT_MATTER table.
	...
	TEST_ASSERT(abs(dq_type_property(/obj/item/dq_property_test/blunt, PROP_MASS) - 0.2) < 0.0001, "a subtype's own DEFAULT_MATTER wins, got [dq_type_property(/obj/item/dq_property_test/blunt, PROP_MASS)]")
	...
	item.set_matter(list(MAT_STEEL = 3000))
