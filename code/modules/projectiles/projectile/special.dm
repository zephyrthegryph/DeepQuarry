...
/obj/item/projectile/bola/on_hit(atom/target, blocked = 0)
	if(ishuman(target))
		...
		for(var/obj/item/clothing/cloth in list(human_target.get_equipped_item(SLOT_ID_WEAR_SUIT), human_target.get_equipped_item(SLOT_ID_W_UNIFORM), human_target.get_equipped_item(SLOT_ID_SHOES))) //Check if we have a thick material covering our feet.
