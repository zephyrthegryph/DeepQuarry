...
/proc/handle_lasertag_attack(target, mob/living/carbon/human/attacker, tag_damage, vest_override, required_vest, list/allowed_suits)
	...
	if(vest_override || (attacker && istype(attacker) && (!required_vest || istype(attacker.get_equipped_item(SLOT_ID_SUIT), required_vest))))
		if(ishuman(target))
			...
			if(is_type_in_list(human_target.get_equipped_item(SLOT_ID_SUIT), allowed_suits))
				var/obj/item/clothing/suit/lasertag/laser_suit = human_target.get_equipped_item(SLOT_ID_SUIT)
...
/obj/item/gun/energy/lasertag/special_check(mob/living/carbon/human/M)
	if(ishuman(M) && !vest_override)
		if(!istype(M.get_equipped_item(SLOT_ID_SUIT), required_vest))
			to_chat(M, span_warning("You need to be wearing your laser tag vest!"))
			return FALSE
		var/obj/item/clothing/suit/lasertag/tag_vest = M.get_equipped_item(SLOT_ID_SUIT)
