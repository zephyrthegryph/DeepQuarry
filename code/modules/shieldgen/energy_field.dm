...
//
// Strength is the field's integrity (damage.md §5), FIELD_INTEGRITY_PER_RENWICK
// integrity to the Renwick, so take_damage(x) costs the field x / 20 Renwicks.
// Below one Renwick the field drops (loses density); it is never destroyed,
// only regenerated by its generator.

#define FIELD_INTEGRITY_PER_RENWICK 20
...
/// Field strength in Renwicks.
/obj/effect/energy_field/proc/get_strength()
	return get_integrity() / FIELD_INTEGRITY_PER_RENWICK

/obj/effect/energy_field/proc/set_strength(renwicks)
	update_integrity(clamp(renwicks * FIELD_INTEGRITY_PER_RENWICK, 0, max_integrity))

/obj/effect/energy_field/proc/set_max_strength(renwicks)
	max_integrity = renwicks * FIELD_INTEGRITY_PER_RENWICK
...
/obj/effect/energy_field/proc/adjust_strength(amount, impact = 1)
	if(amount < 0) // Taking damage.
		...
		take_damage(-amount * FIELD_INTEGRITY_PER_RENWICK, BRUTE, null, FALSE)
	else if(amount > 0) // Healing damage.
		repair_damage(amount * FIELD_INTEGRITY_PER_RENWICK)

/obj/effect/energy_field/on_update_integrity(old_value, new_value)
	. = ..()
...
/obj/effect/energy_field/atom_destruction(damage_flag)
