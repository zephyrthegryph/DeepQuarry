...
// === merged from sif_chomp.dm during hard-fork de-suffix (verified no override-order change) ===
//Sif defines
...
/datum/weather/sif/toxinrain/planet_effect(mob/living/carbon/H)
	if(H.z in holder.our_planet.expected_z_levels)
		...
		var/target_zone = pick(BP_ALL)
		var/amount_blocked = H.armor_against(INJURY_CORROSIVE, target_zone)
		...
		H.injure(INJURY_CORROSIVE, damage, target_zone, flags = INJURE_ARMORED)
		if(show_message)
			to_chat(H, effect_message)
...
/datum/weather/sif/downpourfatal/planet_effect(mob/living/L)
	if(L.z in holder.our_planet.expected_z_levels)
		...
		var/target_zone = pick(BP_ALL)
		var/amount_blocked = L.armor_against(INJURY_BLUNT, target_zone)
