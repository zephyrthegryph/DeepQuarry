...
/obj/item/clothing/glasses/thermal/emp_act(severity, recursive)
	...
	if(ishuman(src.loc))
		...
		if(M.get_equipped_item(SLOT_ID_GLASSES) == src)
			M.Blind(3)
			M.eye_blurry = 5
...
/obj/item/glasses_kit/afterattack(target, mob/living/carbon/human/user, proximity)
	...
	//We're getting a prescription
		...
		if(T.get_equipped_item(SLOT_ID_GLASSES) || (T.get_equipped_item(SLOT_ID_HEAD) && T.get_equipped_item(SLOT_ID_HEAD).flags_inv & HIDEEYES))
