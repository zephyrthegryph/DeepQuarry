	...
	for(var/atom/thing in view(5, holder)) // It's 5 and not 7 so players have a chance to go away before getting the prompts, and for performance.
		...
		if(isliving(thing))
			...
			if(ishuman(thing))
				...
				if(!H.get_equipped_item(SLOT_ID_GLOVES) && H.bloody_hands && H.hand_blood_color != SYNTH_BLOOD_COLOUR)
					human_blood_fear_amount += 1
				if(!H.get_equipped_item(SLOT_ID_SHOES) && H.feet_blood_color && H.feet_blood_color != SYNTH_BLOOD_COLOUR)
					human_blood_fear_amount += 1
		...
		if(istype(thing, /obj))
			var/obj/O = thing
			if(O.forensic_data?.has_blooddna() && dq_get_blood_color(O) && dq_get_blood_color(O) != SYNTH_BLOOD_COLOUR)
				fear_amount++
