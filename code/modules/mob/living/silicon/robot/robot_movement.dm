		...
		for(var/A in tile)
			if(istype(A, /obj/effect))
				...
				if(cleaned_human.lying)
					if(cleaned_human.get_equipped_item(SLOT_ID_HEAD))
						cleaned_human.get_equipped_item(SLOT_ID_HEAD).wash(CLEAN_SCRUB)
						cleaned_human.update_inv_head(0)
					if(cleaned_human.get_equipped_item(SLOT_ID_WEAR_SUIT))
						cleaned_human.get_equipped_item(SLOT_ID_WEAR_SUIT).wash(CLEAN_SCRUB)
						cleaned_human.update_inv_wear_suit(0)
					else if(cleaned_human.get_equipped_item(SLOT_ID_W_UNIFORM))
						cleaned_human.get_equipped_item(SLOT_ID_W_UNIFORM).wash(CLEAN_SCRUB)
						cleaned_human.update_inv_w_uniform(0)
					if(cleaned_human.get_equipped_item(SLOT_ID_SHOES))
						cleaned_human.get_equipped_item(SLOT_ID_SHOES).wash(CLEAN_SCRUB)
