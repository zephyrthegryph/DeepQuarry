/mob/living/carbon/human/proc/handle_strip(slot_to_strip,mob/living/user)
	...
	switch(slot_to_strip)
		...
		if("internals")
			...
			return
		if("tie")
			var/obj/item/clothing/under/suit = get_equipped_item(SLOT_ID_W_UNIFORM)
...
/mob/living/carbon/human/proc/empty_pockets(mob/living/user)
	if(!get_equipped_item(SLOT_ID_R_STORE) && !get_equipped_item(SLOT_ID_L_STORE))
		to_chat(user, span_warning("\The [src] has nothing in their pockets."))
		return
	if(get_equipped_item(SLOT_ID_R_STORE))
		unEquip(get_equipped_item(SLOT_ID_R_STORE))
	if(get_equipped_item(SLOT_ID_L_STORE))
		unEquip(get_equipped_item(SLOT_ID_L_STORE))
...
/mob/living/carbon/human/proc/remove_splints(mob/living/user)
	...
	if(istype(get_equipped_item(SLOT_ID_WEAR_SUIT),/obj/item/clothing/suit/space))
		var/obj/item/clothing/suit/space/suit = get_equipped_item(SLOT_ID_WEAR_SUIT)
...
/mob/living/carbon/human/proc/toggle_internals(mob/living/user)
	if(internal)
		...
		if(!(istype(get_equipped_item(SLOT_ID_WEAR_MASK), /obj/item/clothing/mask) || istype(get_equipped_item(SLOT_ID_HEAD), /obj/item/clothing/head/helmet/space)))
			return
		// Find an internal source.
		if(istype(get_equipped_item(SLOT_ID_BACK), /obj/item/tank))
			internal = get_equipped_item(SLOT_ID_BACK)
		else if(istype(get_equipped_item(SLOT_ID_S_STORE), /obj/item/tank))
			internal = get_equipped_item(SLOT_ID_S_STORE)
