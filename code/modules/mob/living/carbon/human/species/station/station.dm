...
// === merged from station_ch.dm during hard-fork de-suffix (manually verified: all-new types/defines, no base re-open) ===
...
/datum/species/xenomorph_hybrid/handle_environment_special(mob/living/carbon/human/H)
	...
	if((environment.get_moles(GAS_PHORON) > 0) || locate(/obj/effect/alien/weeds) in T)
		if(!regenerate(H))
