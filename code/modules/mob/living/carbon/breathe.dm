...
/mob/living/carbon/proc/get_breath_from_internal(volume_needed=BREATH_VOLUME) //hopefully this will allow overrides to specify a different default volume without breaking any cases where volume is passed in.
	if(internal)
		if (!contents.Find(internal))
			internal = null
		if (!(get_equipped_item(SLOT_ID_WEAR_MASK) && (get_equipped_item(SLOT_ID_WEAR_MASK).item_flags & AIRTIGHT)))
			internal = null
		if(internal)
			if (internals)
...
/datum/life_system/breathing/carbon/proc/breath_from_environment(mob/living/carbon/self, volume_needed=BREATH_VOLUME)
	...
	if(breath)
		...
		if(istype(self.get_equipped_item(SLOT_ID_WEAR_MASK), /obj/item/clothing/mask) && breath)
			var/obj/item/clothing/mask/M = self.get_equipped_item(SLOT_ID_WEAR_MASK)
