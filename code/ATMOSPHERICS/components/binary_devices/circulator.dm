...
/obj/machinery/atmospherics/binary/circulator/proc/return_transfer_air()
	...
	if(anchored && !(stat&BROKEN) && network1)
		...
		if(air1.return_temperature() > 0 && last_pressure_delta > 5)

			//Calculate necessary moles to transfer using PV = nRT
			recent_moles_transferred = (last_pressure_delta*network1.volume/(air1.return_temperature() * R_IDEAL_GAS_EQUATION))/3 //uses the volume of the whole network, not just itself
