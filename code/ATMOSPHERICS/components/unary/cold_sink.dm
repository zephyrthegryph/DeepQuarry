...
/obj/machinery/atmospherics/unary/freezer/tgui_data(mob/user)
	...
	var/temp_class = "good"
	if(air_contents.return_temperature() > (T0C - 20))
		temp_class = "bad"
	else if(air_contents.return_temperature() < (T0C - 20) && air_contents.return_temperature() > (T0C - 100))
		temp_class = "average"
...
/obj/machinery/atmospherics/unary/freezer/process()
	...
	if(network && air_contents.return_temperature() > set_temperature)
		cooling = 1
		...
		var/cop = FREEZER_PERF_MULT * air_contents.return_temperature()/heatsink_temperature	//heatpump coefficient of performance from thermodynamics -> power used = heat_transfer/cop
