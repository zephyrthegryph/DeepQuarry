...
/obj/item/wrapping_paper/attack(mob/living/target, mob/living/user, target_zone, attack_modifier)
	...
	if (istype(H.get_equipped_item(SLOT_ID_SUIT), /obj/item/clothing/suit/straight_jacket) || istype(H.get_equipped_item(SLOT_ID_SUIT), /obj/item/clothing/suit/shibari) || H.stat)
		if (src.amount >= 2)
...
/obj/item/a_gift/advanced/attack_self(mob/M) //WIP - ALWAYS add more items to list! - Jack
	...
	var/gift_type_advanced = pick(
		...
		/obj/item/rig/medical, // Below here is items that need to be upported. // Enable Start
		/obj/item/lego,
	...
	var/gift_type_chaos = pick(
		...
		/obj/item/spellbook,
		/obj/item/book/tome/imbued, // Below here is items that need to be upported. // Enable Start
		/obj/item/grenade/spawnergrenade/casino/infinitycake,
		/obj/item/grenade/spawnergrenade/casino/universal_technomancer) // Enable End
