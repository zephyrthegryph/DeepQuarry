...
/obj/item/card/id/assistant
	assignment = JOB_ALT_VISITOR // Visitor not Assistant
	rank = JOB_ALT_VISITOR // Visitor not Assistant
...
/obj/item/card/id/event/attack_self(mob/user)
	...
	if(title_strings.len)
		...
		name = user.name + "'s ID card" + " ([assignment] Contractor)" // Suffix contractor IDs
