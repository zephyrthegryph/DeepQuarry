...
/obj/item/clothing/mask/smokable/proc/smoke(amount)
	...
	if(reagents && reagents.total_volume) // check if it has any reagents at all
		if(ishuman(loc))
			...
			if (src == C.get_equipped_item(SLOT_ID_WEAR_MASK) && C.check_has_mouth()) // if it's in the human/monkey mouth, transfer reagents to the mob
...
/obj/item/flame/lighter/attack(mob/living/M, mob/living/user, target_zone, attack_modifier)
	...
	if(istype(M.get_equipped_item(SLOT_ID_WEAR_MASK), /obj/item/clothing/mask/smokable/cigarette) && user.zone_sel.selecting == O_MOUTH && lit)
		var/obj/item/clothing/mask/smokable/cigarette/cig = M.get_equipped_item(SLOT_ID_WEAR_MASK)
		if(M == user)
