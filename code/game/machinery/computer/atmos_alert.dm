...
/obj/machinery/computer/atmos_alert/update_icon()
	if(!(stat & (NOPOWER|BROKEN)))
		...
		if(alarms.len)
			icon_screen = "alert:2"
			playsound(src, 'sound/effects/comp_alert_major.ogg', 70, 1)
			...
			if(alarms.len)
				icon_screen = "alert:1"
				playsound(src, 'sound/effects/comp_alert_minor.ogg', 50, 1)
				spawn(100) // Wait 10 seconds, then play it again
					playsound(src, 'sound/effects/comp_alert_minor.ogg', 50, 1)
			else
				icon_screen = initial(icon_screen)
				playsound(src, 'sound/effects/comp_alert_clear.ogg', 50, 1)
	..()
