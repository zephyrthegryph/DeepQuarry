	...
	if(isliving(hit_atom))
		...
		if(L.mob_class & allowed_mob_classes)
			...
			if(pay_energy(25 * length(controlled_mobs)))
				attack_all(L)
				add_attack_logs(user,L,"Commanded their army of [length(controlled_mobs)]")
				to_chat(user, span_notice("You command your [length(controlled_mobs) > 1 ? "entities" : "[LAZYACCESS(controlled_mobs, 1)]"] to \
				attack \the [L]."))
