...
/obj/mecha/combat/gygax/serenity/moved_inside(mob/living/carbon/human/H as mob)
	if(..())
		if(H.get_equipped_item(SLOT_ID_GLASSES))
			occupant_message(span_red("[H.get_equipped_item(SLOT_ID_GLASSES)] prevent you from using [src] [hud]!"))
		else if(hud.move_into(H, SLOT_ID_GLASSES, H))
			H.recalculate_vis()
		return 1
	else
		return 0
...
/obj/mecha/combat/gygax/serenity/go_out()
	if(ishuman(occupant))
		...
		if(H.get_equipped_item(SLOT_ID_GLASSES) == hud)
