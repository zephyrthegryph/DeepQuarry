...
/datum/job/proc/setup_account(mob/living/carbon/human/H)
	...
	if(H.client)
		switch(H.client.prefs.read_preference(/datum/preference/choiced/human/economic_status))
			...
			if(CLASS_BROKE)		income = 0 // Rent's not cheap
...
/datum/job/proc/get_min_age(species_name, brain_type)
	return minimum_character_age // Minimum character age by rules is 18, return default which is standard for all species
	// return (brain_type && LAZYACCESS(min_age_by_species, brain_type)) || LAZYACCESS(min_age_by_species, species_name) || minimum_character_age // Removal

/datum/job/proc/get_ideal_age(species_name, brain_type)
	return ideal_character_age // Minimum character age by rules is 18, return default which is standard for all species
	// return (brain_type && LAZYACCESS(ideal_age_by_species, brain_type)) || LAZYACCESS(ideal_age_by_species, brain_type) || ideal_character_age // Removal

/datum/job/proc/is_species_banned(species_name, brain_type)
	// begin -- Shadekin cannot be any crew position
