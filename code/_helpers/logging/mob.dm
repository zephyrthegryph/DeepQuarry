...
/mob/log_message(message, message_type, color = null, log_globally = TRUE, list/data)
	...
	var/list/timestamped_message = list(
		...
		"ckey" = ckey,
		"loc" = loc_name(src),
		"event_id" = LAZYLEN(LAZYACCESS(logging, smessage_type)),
		"message" = message,
		"color" = color
	)

	if(!CONFIG_GET(flag/database_logging))
		if(!islist(LAZYACCESS(logging, smessage_type)))
			LAZYSET(logging, smessage_type, list())
	...
	if(HAS_CONNECTED_PLAYER(src))
		if(CONFIG_GET(flag/database_logging))
			...
			if(!islist(LAZYACCESS(persistent_client.logging, smessage_type)))
				LAZYSET(persistent_client.logging, smessage_type, list())
