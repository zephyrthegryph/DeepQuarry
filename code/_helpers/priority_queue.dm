...
/datum/priority_queue/proc/peek()
	return length(array)? LAZYACCESS(array, 1) : null

// todo: define this
/datum/priority_queue/proc/bubble_up(index)
	while(index >= 2 && call(comparison)(LAZYACCESS(array, index), LAZYACCESS(array, index / 2)) < 0)
		LAZYINITLIST(array); array.Swap(index, index / 2)
		index /= 2
...
/datum/priority_queue/proc/bubble_down(index)
	...
	while(next <= length)
		...
		if(call(comparison)(LAZYACCESS(array, next), LAZYACCESS(array, index)) < 0)
			if(next < length && call(comparison)(LAZYACCESS(array, next), LAZYACCESS(array, next + 1)) > 0)
				LAZYINITLIST(array); array.Swap(index, next + 1)
				index = next + 1
			else
				LAZYINITLIST(array); array.Swap(index, next)
				index = next
		else if(next < length && call(comparison)(LAZYACCESS(array, next + 1), LAZYACCESS(array, index)) < 0)
			LAZYINITLIST(array); array.Swap(index, next + 1)
			index = next + 1
		else
			break
...
/datum/priority_queue/proc/find(entry)
	return LAZYFIND(array, entry)

/datum/priority_queue/proc/remove_entry(entry)
	return remove_index(LAZYFIND(array, entry))
