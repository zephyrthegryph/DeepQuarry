...
/datum/component/omen/proc/check_slip(mob/living/our_guy, amount)
	...
	if(prob(30 * luck_mod) && our_guy.get_bodypart_name(BP_HEAD)) /// Bonk!
		...
		if(ishuman(our_guy))
			...
			if(human_guy.get_equipped_item(SLOT_ID_GLASSES) && human_guy.canUnEquip(human_guy.get_equipped_item(SLOT_ID_GLASSES)))
				var/turf/T = get_turf(human_guy)
				if(T)
					var/obj/item/our_glasses = human_guy.get_equipped_item(SLOT_ID_GLASSES)
					human_guy.unEquip(human_guy.get_equipped_item(SLOT_ID_GLASSES), target = T)
