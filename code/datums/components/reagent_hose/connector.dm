...
/datum/component/hose_connector/Initialize(set_unique_name = null)
	...
	if(my_hose || reagents.total_volume)
		REACT_PROCESS(src, 2 SECONDS, "pumps reagents through the attached hose every tick while connected")
...
/datum/component/hose_connector/proc/remove_hose()
	my_hose = null
	// Flush the connector immediately, then leave the continuous lane. There is
	// no reason to wait up to one REACT_PROCESS period merely to discover disconnection.
	process()
	REACT_PROCESS_STOP(src)
