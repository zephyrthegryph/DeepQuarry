	...
	if(ishuman(src))
		...
		switch(target_zone)
			if(BP_HEAD)
				...
				if(passed && isobj(H.get_equipped_item(SLOT_ID_WEAR_MASK)))
					Cl = H.get_equipped_item(SLOT_ID_WEAR_MASK)
					passed = prob((Cl.permeability_coefficient*100) - 1)
			if(BP_TORSO)
				if(isobj(H.get_equipped_item(SLOT_ID_WEAR_SUIT)))
					Cl = H.get_equipped_item(SLOT_ID_WEAR_SUIT)
					passed = prob((Cl.permeability_coefficient*100) - 1)
				if(passed && isobj(H.get_equipped_item(SLOT_ID_W_UNIFORM)))
					Cl = H.get_equipped_item(SLOT_ID_W_UNIFORM)
					passed = prob((Cl.permeability_coefficient*100) - 1)
			if(BP_L_HAND, BP_R_HAND)
				if(isobj(H.get_equipped_item(SLOT_ID_WEAR_SUIT)) && H.get_equipped_item(SLOT_ID_WEAR_SUIT).body_parts_covered & HANDS)
					Cl = H.get_equipped_item(SLOT_ID_WEAR_SUIT)
					passed = prob((Cl.permeability_coefficient*100) - 1)
			...
			if(BP_L_FOOT, BP_R_FOOT)
				if(isobj(H.get_equipped_item(SLOT_ID_WEAR_SUIT)) && H.get_equipped_item(SLOT_ID_WEAR_SUIT).body_parts_covered & FEET)
					Cl = H.get_equipped_item(SLOT_ID_WEAR_SUIT)
					passed = prob((Cl.permeability_coefficient*100) - 1)
