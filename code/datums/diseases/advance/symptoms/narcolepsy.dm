...
/datum/symptom/narcolepsy/Activate(datum/disease/advance/A)
	...
	switch(sleep_level)
		if(10 to 19)
			M.status_adjust(EFFECT_DROWSY, 1)
		if(20 to INFINITY)
			M.status_adjust(EFFECT_SLEEPING, 30)
			sleep_level = 0
			sleepy_ticks = 0
	...
	switch(A.stage)
		...
		if(3)
			if(prob(15))
				...
				sleepy_ticks += rand(10, 14)
				if(weakens)
					M.status_adjust(EFFECT_WEAKENED, 15)
		if(4)
			if(prob(20))
				to_chat(M, "You nod off for a moment.")
				sleepy_ticks += rand(10, 14)
				if(weakens)
					M.status_adjust(EFFECT_WEAKENED, 20)
		if(5)
			if(prob(25))
				...
				M.status_at_least(EFFECT_DROWSY, 2)
				sleepy_ticks += rand(10, 14)
				if(weakens)
					M.status_adjust(EFFECT_WEAKENED, 30)
