		...
		with_turf_gases_read(move |arena| -> Result<Vec<(ByondValue, ByondValue)>> {
			...
			for &cur_index in progression_order.iter().rev() {
				...
				#[cfg(feature = "katmos_slow_decompression")]
				{
					...
					// DQEdit: was `clear_vol` upstream — that method doesn't exist on TurfMixture
					// (rustc suggests `clear_moles`, matching the variable name and the parallel
					// `clear_air` branch above). Upstream this when convenient.
