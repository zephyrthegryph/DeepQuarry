//! Cellular-automata cave generator used by expedition sites. The DM bind lives
//! in `vg-ffi` (`generate_automata`).
...
/// Validate DM's (limit_x, limit_y, iterations, initial_wall_cell) and return a
/// flat row-major wall map (`true` = wall) of length `limit_x * limit_y`.
pub fn generate_automata(
    limit_x: f64,
    limit_y: f64,
    iterations: f64,
    initial_wall_cell: f64,
) -> Result<Vec<bool>> {
    let limit_x = validate_count(limit_x, "limit_x", MAX_DIMENSION)?;
    let limit_y = validate_count(limit_y, "limit_y", MAX_DIMENSION)?;
    let iterations = validate_count(iterations, "iterations", MAX_ITERATIONS)?;
    let initial_wall_cell = validate_count(initial_wall_cell, "initial_wall_cell", 100)?;
    Ok(smooth_map(
