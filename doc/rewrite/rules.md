...
- **Running.** Rules are singletons. An object subscribes when it materializes (until L2's `on_materialize()` lands, `SSatoms.InitAtom` calls `dq_rules_on_materialize()`), getting one `/datum/rule_binding`. Every wake re-checks the whole predicate; a rule fires on the false-to-true edge. `hold_for` runs on a rate model with a `REACT_RATE` watch, never polling.
- **Reactor coupling.** Only `reactor_adapter.dm` touches SSreactor. Objects have no heat-domain node yet, so a heat node is DM-mirrored and borrows one of S1's probe cells while it is away from ambient; at rest, or when the cells run out, its watches fall back to a DM-owned key.
