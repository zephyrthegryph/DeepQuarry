...
- `bin/build.cmd` validates DME reachability and builds changed components.
...
- `bin/test.cmd` compiles and boots the unit-test world. A nonempty
  `code/modules/unit_tests/dq_focus.dm` deliberately selects a focused run and
  must be empty in merge-ready commits.
