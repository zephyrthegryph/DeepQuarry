...
CHOMPStation2 is a hard fork in the Space Station 13 lineage Baystation12 → Polaris → VOREStation → Yawn-wider → CHOMPStation. It is now fully separated and no longer tracks or merges from any upstream, while keeping some features inherited from that lineage.
...
Finally, to start the server, run Dream Daemon and enter the path to your compiled deepquarry.dmb file. Make sure to set the port to the one you specified in the config.txt, and set the Security box to 'Trusted'. Then press GO and the server should start up and be ready to join.
